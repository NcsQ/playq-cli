  ...
  try {
    ...
    if (opts.listReleases) {
      ...
      return;
        ...
        // If extracting to current directory, check for conflicts
        if (opts.dir === ".") {
          // Files that should be safely ignored (won't be overwritten by PlayQ)
          const safeToIgnore = [
            'node_modules',
            '.git',
            '.gitignore',
          ...
          // Critical files that need conflict resolution
          const criticalFiles = [
            'package.json',
            'package-lock.json',
            'yarn.lock',
          ...
          const conflicts = criticalFiles.filter(file => fs.existsSync(path.join(cwd, file)));
          
          if (conflicts.length > 0) {
            log.warn(`⚠️  Critical files detected that may be overwritten:`);
            conflicts.forEach(file => log.warn(`   - ${file}`));
            ...
            const backupChoice = await new Promise((resolve) => {
              rl.question(
                `\nCreate backups before installation? (recommended) (y/n): `,
                (answer) => {
                  rl.close();
                  resolve(answer.trim().toLowerCase());
                }
            ...
            if (backupChoice === "y" || backupChoice === "yes") {
              // Create backups
              const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
              const backupDir = path.join(cwd, `.playq-backup-${timestamp}`);
              fs.mkdirSync(backupDir);
              
              log.info(`📁 Creating backups in ${backupDir}/`);
          ...
          // Check for other blocking files
          const currentDirContents = fs.readdirSync(cwd).filter(item => 
            !item.startsWith('.') && 
            !safeToIgnore.includes(item) &&
            !criticalFiles.includes(item)
          );
          
          if (currentDirContents.length > 0) {
            log.error(`Current directory has files that may conflict with PlayQ installation.`);
            log.info(`Safe to ignore: ${safeToIgnore.join(', ')}`);
            log.info(`Will backup if requested: ${criticalFiles.join(', ')}`);
            log.info(`Found conflicting: ${currentDirContents.join(', ')}`);
            log.info(`Please run from an empty directory or specify --dir <folder>`);
            process.exit(1);
          }
        }
        ...
        // Smart merge package.json if backups were created and installing to current directory
        if (opts.dir === ".") {
          await smartMergePackageJson(targetDir);
        }
...
async function smartMergePackageJson(targetDir) {
  try {
    // Look for backup directories
    const backupDirs = fs.readdirSync(targetDir).filter(item => 
      item.startsWith('.playq-backup-') && fs.statSync(path.join(targetDir, item)).isDirectory()
    );
    
    if (backupDirs.length === 0) {
      return; // No backups, nothing to merge
    }
    
    // Use the most recent backup
    const latestBackup = backupDirs.sort().pop();
    const backupDir = path.join(targetDir, latestBackup);
    const backupPkgPath = path.join(backupDir, 'package.json');
    const currentPkgPath = path.join(targetDir, 'package.json');
    
    if (!fs.existsSync(backupPkgPath) || !fs.existsSync(currentPkgPath)) {
      return; // No package.json files to merge
    }
    
    const backupPkg = JSON.parse(fs.readFileSync(backupPkgPath, 'utf8'));
    const playqPkg = JSON.parse(fs.readFileSync(currentPkgPath, 'utf8'));
    
    // Smart merge strategy
    const mergedPkg = {
      ...playqPkg, // Start with PlayQ package.json
      name: backupPkg.name || playqPkg.name, // Preserve original project name
      version: backupPkg.version || playqPkg.version, // Preserve original version
      description: backupPkg.description || playqPkg.description, // Preserve original description
      
      // Merge dependencies (prioritize original CLI dependencies)
      dependencies: {
        ...playqPkg.dependencies,
        ...backupPkg.dependencies, // Original dependencies override PlayQ ones
      },
      
      devDependencies: {
        ...playqPkg.devDependencies,
        ...backupPkg.devDependencies, // Original dev dependencies override PlayQ ones
      },
      
      // Merge scripts (keep both original and PlayQ scripts)
      scripts: {
        ...playqPkg.scripts,
        ...backupPkg.scripts, // Original scripts override PlayQ ones in case of conflicts
      },
      
      // Preserve original metadata
      author: backupPkg.author || playqPkg.author,
      license: backupPkg.license || playqPkg.license,
      keywords: [...(playqPkg.keywords || []), ...(backupPkg.keywords || [])].filter((v, i, a) => a.indexOf(v) === i), // Unique keywords
    };
    
    // Write merged package.json
    fs.writeFileSync(currentPkgPath, JSON.stringify(mergedPkg, null, 2));
    
    log.info(`🔧 Smart merged package.json:`);
    log.info(`   ✅ Preserved original project name: ${backupPkg.name}`);
    log.info(`   ✅ Kept CLI dependencies: @playq/cli`);
    log.info(`   ✅ Added PlayQ dependencies: ${Object.keys(playqPkg.devDependencies || {}).length} dev packages`);
    log.info(`   ✅ Merged scripts: ${Object.keys(mergedPkg.scripts).length} total scripts`);
    
  } catch (error) {
    log.warn(`⚠️  Could not auto-merge package.json: ${error.message}`);
    log.info(`   You can manually merge from the backup in ${backupDirs[0] || '.playq-backup-*'}/`);
  }
}
