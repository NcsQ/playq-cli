...
export default function (program) {
  const init = program
    .command("init")
    .description("Initialise PlayQ resources");

  init
    .command("base")
    .description("Initialise the PlayQ Base Project folder structure")
    .option("--dir <name>", "target directory name", DEFAULT_DIR)
    .option("--release <version>", "PlayQ Base version or release to install", "latest")
    .option("--offline <path>", "Path to local PlayQ Base zip file")
    ...
    .action(async (opts) => {
      try {
        ...
        if (cwd.includes("playq-cli")) {
          ...
          log.info(
            'Please run "playq init base" from your project root or workspace instead.\n'
          );
          return;
        }
        ...
        if (opts.offline) {
          ...
          spinner.start(`Extracting PlayQ Base from offline package${opts.dir === "." ? " to current directory" : ` → ${opts.dir}`}`);
        } else {
          ...
          spinner.start(`Extracting PlayQ Base${opts.dir === "." ? " to current directory" : ` → ${opts.dir}`}`);
        }
        ...
        spinner.succeed("PlayQ Base extracted successfully");
...
async function listAvailableReleases() {
  try {
    ...
    log.info(`\nUsage:`);
    log.info(`  playq init base                    # Install latest (v${latestVersion})`);
    log.info(`  playq init base --release 1.0.0    # Install specific version`);
    log.info(`  playq init base --release latest   # Install latest explicitly\n`);
